...
export default props => {
  ...
  // List of potential professors
  const [professors] = useState(["Todos", "Pedro Lopez", "Maria Hernandez", "Sergio Urbina"]);
  ...
  useEffect(() => {
    ...
    if (selection.time && selection.date) {
      (async _ => {
        ...
        res = await res.json();
        // Filter available tables with professor and group size criteria
        let tables = res.tables.filter(
          table =>
            (selection.size > 0 ? table.capacity >= selection.size : true) &&
            (selection.professor !== "Todos"
              ? table.professor === selection.professor
              : true)
        );
  ...
  }, [selection.time, selection.date, selection.size, selection.professor]);
  ...
  // Generate professors dropdown
  const getProfessors = _ => {
    let newProfessors = [];
      ...
      <Row noGutters className="text-center align-items-center pizza-cta">
        <Col>
          <p className="looking-for-pizza">
            {!selection.table.id ? "Registra tu salon" : "Confirma tu registro"}
            <i
              className={
                !selection.table.id
          ...
          <p className="selected-table">
            {selection.table.id
              ? "Estas registrandote en el salón " + selection.table.name
              : null}
          </p>
            ...
            <Col xs="12" sm="3">
              <UncontrolledDropdown>
                <DropdownToggle color="none" caret className="booking-dropdown">
                  {selection.time === null ? "Selecciona horario" : selection.time}
                </DropdownToggle>
                ...
                getEmptyTables() > 0 ? (
                  <div>
                    <div className="table-key">
                      ...
                      &nbsp;&nbsp;
                      <span className="full-table"></span> &nbsp; No Disponible
                      &nbsp;&nbsp;
                    </div>
                    <Row noGutters>{getTables()}</Row>
                  </div>
                ) : (
                  <p className="table-display-message">No hay salones disponibles</p>
                )
              ) : (
                <p className="table-display-message">
                  Favor de seleccionar el horario de su registro.
                </p>
              )}
            </Col>
          </Row>
